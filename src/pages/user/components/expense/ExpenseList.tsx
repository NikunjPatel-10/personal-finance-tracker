...
interface IProps {
  ...
  openEditExpenseModal: (transaction: ITransactionDetail) => void;
  userId: number;
}
...
export default function ExpenseList({
  ...
  const [deleteExpenseTransaction] = useDeleteIncomeExpenseDataMutation();
  ...
  const { data: res } = useGetIncomeExpenseDataQuery({
    userId: userId,
    categoryTypeId: CategoryTypeEnum.Expense,
    pageSize,
  ...
  return (
    ...
    >
      ...
      <Paper radius="md" withBorder shadow="md" style={{ overflow: 'auto' }}>
        {/* Display expense list using table */}
        <DataTable
          columns={expenseTableDataColumns}
          data={dataList}
          page={CategoryType.EXPENSE}
          isResponse={true}
          onEditTransaction={(transaction) => openEditExpenseModal(transaction)}
